...
(function(e) {
  ...
  var obj = {
    ...
    showMoreHandler: function() {
      ...
      e.ajax({
        ...
        success: function(data) {
          ...
          if (e(data).find(".sp-product-listing").length > 0) {
            e(data).find(".sp-product-listing > div").each(function() {
              e('.sp-product-listing').append(e(this));
            });
      ...
      if (e(".filtres-remove-js").length > 0) {
        e('.filtres-remove-js a').unbind().click(function(event) {
          ...
          if($_this.hasClass('clear_all')) {
            ...
            var tag = href.split('/collections/'+page).pop().split('/').pop().split('?view=').shift().split('&view=').shift();
    ...
    pageData: function(data) {
      var current_class = ".sp-product-listing";
      ...
      e(current_class).each(function(index) {
      	var $_this = e(this);
        var $_parent_group = $_this.closest('.sp-collapse');
        
        content = e(data).find(current_class+':eq('+index+')');
        $_this.replaceWith(content);
        content = e(data).find(current_class+':eq('+index+')').closest('.sp-collapse');
      ...
      $('.sp-filters-options .sp-quantity').length && $('.sp-filters-options .sp-quantity').find('.active').trigger('click');
